@include('layouts.header')
    ...
    <div class="album py-5 bg-light">
        ...
        <div class="buttons">
            @if($offset != 0)
                <a href="{{route('index',$offset - 20)}}" class="btn btn-danger" style="float: left;">Previous</a>
            @else
                <a href="{{route('index', $offset - 20)}}" class=""></a>
            @endif

            @if ($total >= $offset + 20)
                <a href="{{route('index', $offset + 20)}}" class="btn btn-danger" style="float: right;">Next</a>
            @endif
        </div>
    </div>
